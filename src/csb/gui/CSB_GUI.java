...
public class CSB_GUI implements CourseDataView {
    ...
    // THIS HANDLES TO ADD OR EDIT LECTURE
    ...
    ComboBox courseSessionComboBox;
    ...
    VBox hwPane;
    VBox hwInfoPane;
    Label hwInfoHeadingLabel;
    SplitPane splitHwInfoPane;
    ...
    // LECTURE ITEMS
    VBox lectureItemsBox;
    HBox lectureItemsToolbar;
    Button addLectureItemButton;
    Button removeLectureItemButton;
    Label lectureItemsLabel;
    TableView <Lecture> lectureItemsTable;
    TableColumn itemTopicColumn;
    TableColumn itemNumSessionColumn;
    
    // HW ITEMS
    VBox hwItemsBox;
    HBox hwItemsToolbar;
    Button addHwItemButton;
    Button removeHwItemButton;
    Label hwItemsLabel;
    TableView <Assignment> hwItemsTable;
    TableColumn itemNameColumn;
    TableColumn itemhwTopicColumn;
    TableColumn itemhwDateColumn;
    ...
    static final String COL_HW_DATE = "Date";
    ...
    private void initLectureItemsControls(){
        // NOW THE CONTROLS FOR ADDING SCHEDULE ITEMS
        lectureItemsBox = new VBox();
        lectureItemsToolbar = new HBox();
        lectureItemsLabel = initLabel(CSB_PropertyType.LECTURES_HEADING_LABEL, CLASS_SUBHEADING_LABEL);
        addLectureItemButton = initChildButton(lectureItemsToolbar, CSB_PropertyType.ADD_ICON, CSB_PropertyType.ADD_ITEM_TOOLTIP, false);
        removeLectureItemButton = initChildButton(lectureItemsToolbar, CSB_PropertyType.MINUS_ICON, CSB_PropertyType.REMOVE_ITEM_TOOLTIP, false);
        upItemButton = initChildButton(lectureItemsToolbar, CSB_PropertyType.MOVE_UP_ICON, CSB_PropertyType.MOVE_UP_LECTURE_TOOLTIP, false);
        downItemButton = initChildButton(lectureItemsToolbar, CSB_PropertyType.MOVE_DOWN_ICON, CSB_PropertyType.MOVE_DOWN_LECTURE_TOOLTIP, false);
        lectureItemsTable = new TableView();
        lectureItemsBox.getChildren().add(lectureItemsLabel);
        lectureItemsBox.getChildren().add(lectureItemsToolbar);
        lectureItemsBox.getChildren().add(lectureItemsTable);
        lectureItemsBox.getStyleClass().add(CLASS_BORDERED_PANE);
        
        // NOW SETUP THE TABLE COLUMNS
        itemTopicColumn = new TableColumn(COL_TOPIC);
        itemNumSessionColumn = new TableColumn(COL_SESSIONS);
        
        
        // AND LINK THE COLUMNS TO THE DATA
        itemTopicColumn.setCellValueFactory(new PropertyValueFactory<String, String>("topic"));
        itemNumSessionColumn.setCellValueFactory(new PropertyValueFactory<LocalDate, String>("sessions"));
        
        lectureItemsTable.getColumns().add(itemTopicColumn);
        lectureItemsTable.getColumns().add(itemNumSessionColumn);
        
        lectureItemsTable.setItems(dataManager.getCourse().getLectures());
        //lectureItemsTable.setItems(dataManager.getCourse().getLectures());
        // NOW LET'S ASSEMBLE ALL THE CONTAINERS TOGETHER
        ...
        // FIRST OUR SCHEDULE HEADER
        //lectureInfoHeadingLabel = initChildLabel(lectureInfoPane, CSB_PropertyType.SCHEDULE_HEADING_LABEL, CLASS_HEADING_LABEL);
        ...
        // FINALLY, EVERYTHING IN THIS REGION ULTIMATELY GOES INTO schedulePane
        //schedulePane = new VBox();
        //schedulePane.getChildren().add(lectureInfoPane);
        schedulePane.getChildren().add(lectureItemsBox);
        schedulePane.getStyleClass().add(CLASS_BORDERED_PANE);
    ...
    private void initHwItemsControls(){
        // NOW THE CONTROLS FOR ADDING SCHEDULE ITEMS
        hwItemsBox = new VBox();
        hwItemsToolbar = new HBox();
        hwItemsLabel = initLabel(CSB_PropertyType.HWS_HEADING_LABEL, CLASS_SUBHEADING_LABEL);
        addHwItemButton = initChildButton(hwItemsToolbar, CSB_PropertyType.ADD_ICON, CSB_PropertyType.ADD_ITEM_TOOLTIP, false);
        removeHwItemButton = initChildButton(hwItemsToolbar, CSB_PropertyType.MINUS_ICON, CSB_PropertyType.REMOVE_ITEM_TOOLTIP, false);
        
        hwItemsTable = new TableView();
        hwItemsBox.getChildren().add(hwItemsLabel);
        hwItemsBox.getChildren().add(hwItemsToolbar);
        hwItemsBox.getChildren().add(hwItemsTable);
        hwItemsBox.getStyleClass().add(CLASS_BORDERED_PANE);
        
        // NOW SETUP THE TABLE COLUMNS
        itemNameColumn = new TableColumn(COL_NAME);
        itemhwTopicColumn = new TableColumn(COL_TOPICS);
        itemhwDateColumn = new TableColumn(COL_HW_DATE);
        
         //AND LINK THE COLUMNS TO THE DATA
        itemNameColumn.setCellValueFactory(new PropertyValueFactory<String, String>("name"));
        itemhwTopicColumn.setCellValueFactory(new PropertyValueFactory<LocalDate, String>("topics"));
        itemhwDateColumn.setCellValueFactory(new PropertyValueFactory<LocalDate, String>("date"));
        hwItemsTable.getColumns().add(itemNameColumn);
        hwItemsTable.getColumns().add(itemhwTopicColumn);
        hwItemsTable.getColumns().add(itemhwDateColumn);
        hwItemsTable.setItems(dataManager.getCourse().getAssignments());
        
        
    
        
        schedulePane.getChildren().add(hwItemsBox);
       schedulePane.getStyleClass().add(CLASS_BORDERED_PANE);
    ...
    private void initEventHandlers() throws IOException {
        ...
        progressBar= new ProgressBarDialog(primaryStage,"Done",dataManager.getCourse());
        ...
        // AND NOW THE LECTURE ITEMS
        
        
        addLectureItemButton.setOnAction(e -> {
            lectureController.handleAddLectureItemRequest(this);  //wefewfewffew
        });
        removeLectureItemButton.setOnAction(e -> {
            lectureController.handleRemoveLectureItemRequest(this, lectureItemsTable.getSelectionModel().getSelectedItem());
        });
        upItemButton.setOnAction(e -> {
            Lecture si = lectureItemsTable.getSelectionModel().getSelectedItem();
            lectureController.handleMoveUpLectureItemRequest(this, si);
        });
        downItemButton.setOnAction(e -> {
            Lecture si = lectureItemsTable.getSelectionModel().getSelectedItem();
            lectureController.handleMoveDownLectureItemRequest(this, si);
        });
        addHwItemButton.setOnAction(e -> {
           hwController.handleAddHwItemRequest(this);
        });
        removeHwItemButton.setOnAction(e -> {
            hwController.handleRemoveHwItemRequest(this, hwItemsTable.getSelectionModel().getSelectedItem());
        });
        ...
        // AND NOW THE SCHEDULE ITEMS TABLE
        lectureItemsTable.setOnMouseClicked(e -> {
            if (e.getClickCount() == 2) {
                // OPEN UP THE SCHEDULE ITEM EDITOR
                Lecture si = lectureItemsTable.getSelectionModel().getSelectedItem();
                lectureController.handleEditLectureItemRequest(this, si);
            }
        });
        hwItemsTable.setOnMouseClicked(e -> {
            if (e.getClickCount() == 2) {
                // OPEN UP THE SCHEDULE ITEM EDITOR
                Assignment si = hwItemsTable.getSelectionModel().getSelectedItem();
                hwController.handleEditHwItemRequest(this, si);
            }
        });
